...
import { RecaptchaWrapper } from '@/components/public/RecaptchaWrapper';
  ...
  const handleGoogleRegister = async () => {
    ...
    try {
      ...
      await fetch('/api/reader/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: result.user.email, name: result.user.displayName || 'User' }),
      });
      router.push('/dashboard');
    } catch (err: any) {
      ...
      <div className="min-h-[80vh] flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <p className="text-sm text-muted-foreground mt-2">Join ZaleN-Gam for personalized news</p>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <form onSubmit={handleEmailRegister} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <Lock className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
                  <Input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Confirm password" className="pl-10" required />
                </div>
              </div>
